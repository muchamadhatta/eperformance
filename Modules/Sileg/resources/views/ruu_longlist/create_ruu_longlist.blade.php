                        ...
                        <div class="d-flex flex-row gap-2">
                            <select class="form-select w-50" name="id_periode_prolegnas" id="id_periode_prolegnas">
                                ...
                                @foreach ($prolegnass as $data)
                                    <option value="{{ $data->id }}" @if ($data->id == $ruu_longlist->id_periode_prolegnas) selected @endif>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="d-flex flex-row gap-2">
                            <select class="form-select w-50" name="revisi" id="revisi">
                                ...
                                @for ($i = 0; $i <= 10; $i++)
                                    <option value="{{ $i }}" @if ($i == $ruu_longlist->revisi) selected @endif>{{ $i }}</option>
                                @endfor
                            </select>
                            <font style="color: red; display: flex; align-items: flex-end; padding: 0;">*</font>
                        </div>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            checkboxes.forEach(function(checkbox) {
                checkbox.addEventListener('change', function() {
                    ...
                    const inputs = document.querySelectorAll('input[name^="no_urut_longlist[' +
                        index + ']"], input[name^="judul_ruu_longlist[' + index +
                        ']"], input[name^="pengusul_longlist[' + index + ']"]');
