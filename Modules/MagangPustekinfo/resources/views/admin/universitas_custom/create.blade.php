...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <div>
        ...
        <h4 class="main-title mb-0">Tambah Universitas Custom</h4>
    </div>
</div>

<div class="card">
    <div class="card-body">
        <form action="{{ route('magangpustekinfo.admin.universitas_custom.store') }}" method="POST">
            @csrf
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('group') is-invalid @enderror" id="group" name="group">
                            ...
                            <option value="PTN" {{ old('group') == 'PTN' ? 'selected' : '' }}>PTN (Perguruan Tinggi Negeri)</option>
                            <option value="PTS" {{ old('group') == 'PTS' ? 'selected' : '' }}>PTS (Perguruan Tinggi Swasta)</option>
                        </select>
                        @error('group')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('university_type') is-invalid @enderror" id="university_type" name="university_type">
                            ...
                            <option value="Universitas" {{ old('university_type') == 'Universitas' ? 'selected' : '' }}>Universitas</option>
                            <option value="Institut" {{ old('university_type') == 'Institut' ? 'selected' : '' }}>Institut</option>
                            <option value="Politeknik" {{ old('university_type') == 'Politeknik' ? 'selected' : '' }}>Politeknik</option>
                            <option value="Sekolah Tinggi" {{ old('university_type') == 'Sekolah Tinggi' ? 'selected' : '' }}>Sekolah Tinggi</option>
                            <option value="Akademi" {{ old('university_type') == 'Akademi' ? 'selected' : '' }}>Akademi</option>
                        </select>
                ...
                <div class="col-12">
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('address') is-invalid @enderror" 
                                  ...
                                  placeholder="Alamat lengkap universitas">{{ old('address') }}</textarea>
                        @error('address')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('regency') is-invalid @enderror" 
                               id="regency" name="regency" value="{{ old('regency') }}" 
