    ...
    <div class="card">
        <div class="card-body">
            ...
            <div class="mb-3">
                ...
                <form action="{{ route('setjenweb.publikasi.update', $publikasi->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="d-flex flex-row gap-2">
                            <select class="form-select w-50" name="id_website_menu" id="id_website_menu">
                                ...
                                @foreach ($website_menus as $data)
                                    <option value="{{ $data->id }}" {{ $data->id == $publikasi->id_website_menu ? 'selected' : '' }}>{{ $data->param }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="mb-3">
                        ...
                        @if ($publikasi->cover_file_name)
                            <br>
                            <img class="mb-3 border " src="{{ asset($publikasi->cover_file_name) }}" alt="Gambar" style="max-height: 100px;">
                            <br>
                            <a href="{{ route('setjenweb.publikasi.deleteFile', ['id' => $publikasi->id, 'jenis' => 'img']) }}" class="btn btn-danger"><i class="ri-delete-bin-line"></i> Hapus</a>
                        @else
                    ...
                    <div class="mb-3">
                        ...
                        @if ($publikasi->file_name)
                            <br>
                            <a href="{{ asset($publikasi->file_name) }}" target="_blank"
                                class="btn btn-warning "><i class="ri-file-line"></i>
                                Tampilkan Dokumen</a>

                            <a href="#" class="btn btn-success">
                                <i class="ri-file-line"></i>
                                {{ ceil($publikasi->file_size / 1024) }} KB
                            </a>

                            <a href="{{ route('setjenweb.publikasi.deleteFile', ['id' => $publikasi->id, 'jenis' => 'pdf']) }}"
                        ...
                        <div class="d-flex flex-row gap-2">
                            <select class="form-select w-50" name="id_jenis_dokumen" id="id_jenis_dokumen">
                                ...
                                @foreach ($jenis_dokumens as $data)
                                    <option value="{{ $data->id }}" {{ $data->id == $publikasi->id_jenis_dokumen ? 'selected' : '' }}>{{ $data->jenis_dokumen }}</option>
                                @endforeach
                            </select>
                            <font style="color: red; display: flex; align-items: flex-end; padding: 0;">*</font>
                        </div>
