...
@section('content')
    <div class="d-sm-flex align-items-center justify-content-between mb-4 ">
        ...
        <div>
            <a href="{{ route('setjenweb.publikasi.create') }}" class="btn btn-success"><i
                    class="ri-pencil-line"></i> Tambah Publikasi</a>
        </div>
    </div><!-- row -->
    <div class="card ">
        <div class="card-body">
                ...
                <table id="tableGrid3">
                    <thead>
                        <tr>
                            ...
                            <th scope="col" class="p-1 text-center" style="width: 25%;">Cover Dokumen</th>
                            <th scope="col" class="p-1 text-center" style="width: 25%;">Judul</th>
                            <th scope="col" class="p-1 text-center" style="width: 25%;">Menu</th>
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    @if ($publikasi->cover_file_name)
                                        <img src="{{ asset($publikasi->cover_file_name) }}" alt="Album"
                                            style="max-width: 100px; max-height: 100px;">
                                    @endif
                                </td>
                                <td class="text-center">{{ $publikasi->judul }}</td>
                                <td class="text-center">
                                    @if ($publikasi->id_website_menu)
                                        @php
                                            $menu = App\Models\Setjen\WebsiteMenu::find($publikasi->id_website_menu)->param;
                                        @endphp
                                        {{ $menu }}
                                    @endif
                                </td>
                                <td class="text-center">
                                    <a href="{{ route('setjenweb.publikasi.edit', $publikasi->id) }}"
                                        class="btn btn-primary"><i class="ri-edit-2-line"></i> Edit</a>
                                    <form action="{{ route('setjenweb.publikasi.destroy', $publikasi->id) }}"
