...
@section('content')
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <div>
            ...
            <h4 class="main-title mb-0">Edit Sdm</h4>
        </div>
    </div>
    <div class="card">
        <div class="card-body">
            ...
            <div class="mb-3">
                ...
                <form action="{{ route('setjenweb.sdm.update', $sdm->id) }}" method="POST"
                    ...
                    <div class="mb-3">
                        ...
                        <div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="type" id="circle" value="circle" {{ $sdm->type == 'circle' ? 'checked' : '' }}>
                                <label class="form-check-label" for="circle">
                                    <i class="ri-pie-chart-fill"></i>
                                </label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="type" id="block" value="block" {{ $sdm->type == 'block' ? 'checked' : '' }}>
                        ...
                        <label for="id_menu" class="form-label fw-bold">Menu</label>
                            <select class="form-select w-50" name="id_menu" id="id_menu">
                                ...
                                @foreach ($menus as $data)
                                    <option value="{{ $data->id }}" {{ $data->id == $sdm->id_menu ? 'selected' : '' }}>{{ $data->judul }}</option>
                                @endforeach
                            </select>
                    </div>
